...
os.environ["TRITON_INTERPRET"] = "1"  # Uncomment to enable debug mode
...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    # 这个阶段处理的数值范围
    if STAGE == 1:
        lo, hi = 0, start_m * BLOCK_M # 0, 0*64  lo  - hi 指定k 的seq len维度上的范围
    elif STAGE == 2:
        lo, hi = start_m * BLOCK_M, (start_m + 1) * BLOCK_M
        lo = tl.multiple_of(lo, BLOCK_M) #告知编译器 lo 中的所有值都是BLOCK_M的倍数。
    # causal = False
    else:
        lo, hi = 0, N_CTX
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        if STAGE == 2: #处理对角线元素
            ...
            # tl.maximum 函数返回两个输入张量中每个元素的最大值。
            qk -= m_ij[:, None]
        else: #处理非对角线元素不需要加mask
...
def _attn_fwd(Q, K, V, sm_scale, M, Out,  #
              ...
              BLOCK_M: tl.constexpr,  # Q O的块大小
              BLOCK_N: tl.constexpr,  # K V的块大小
              STAGE: tl.constexpr  #
              ):
    tl.static_assert(BLOCK_N <= HEAD_DIM)
    start_m = tl.program_id(0) # 那个块
    off_hz = tl.program_id(1) # bs * head
    off_z = off_hz // H ## 那个batch
    off_h = off_hz % H ## 那个head
    ...
    Q_block_ptr = tl.make_block_ptr(
        ...
        offsets=(start_m * BLOCK_M, 0),#start_m * BLOCK_M = 第几个块 * 块大小
        block_shape=(BLOCK_M, HEAD_DIM),
        order=(1, 0),
    )
    v_order: tl.constexpr = (0, 1) if V.dtype.element_ty == tl.float8e5 else (1, 0)
    # 内层需要遍历完整的K和V，所以此处K，V的offset =（0，0）
    V_block_ptr = tl.make_block_ptr(
        base=V + qvk_offset,
        shape=(N_CTX, HEAD_DIM),
        strides=(stride_vk, stride_vn), # 保留原来的顺序
        offsets=(0, 0),
    ...
    K_block_ptr = tl.make_block_ptr(
        ...
        strides=(stride_kk, stride_kn),#stride 交换位置了，相当于转置
        offsets=(0, 0),
    ...
    O_block_ptr = tl.make_block_ptr(
        ...
        offsets=(start_m * BLOCK_M, 0),# 块本身对应seq len维度，BLOCK_M是一个块有多少个token
        block_shape=(BLOCK_M, HEAD_DIM),
        order=(1, 0),
    )
    # initialize offsets
...
class _attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, causal, sm_scale):#ctx参数在apply函数中传入，指定类本身
        # shape constraints
        # 形状约束
        ...
        if is_hip():
            ...
            extra_kern_args = {"waves_per_eu": waves_per_eu, "allow_flush_denorm": True}
                            ...
                            #( seq_len//BLOCK_M,bs*head,1)  BLOCK_M 通过 @triton.autotune自动获取了
    ...
    def backward(ctx, do):
        ...
        _attn_bwd_preprocess[pre_grid]( #先计算delta
            o, do,  #
            delta,  #
            BATCH, N_HEAD, N_CTX,  #
...
def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, causal, mode, provider, device="cuda"):
    ...
    if "triton" in provider:
        ...
        if mode == "fwd" and "fp8" in provider:
            ...
            v = v.permute(0, 1, 3, 2)
            v = v.to(torch.float8_e5m2) # 使用fp8的时候，v的内存布局修改了了列优先
        sm_scale = 1.3
        ...
        # 先热身warmup次，然后进行rep次测试，返回平均时间ms
    if provider == "flash":
